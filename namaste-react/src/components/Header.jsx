    ...
    <div className='flex justify-between items-center bg-green-100 bg-gradient-to-tr p-4 shadow-lg mb-2 px-2'>
      ...
      <div className='p-8 flex items-center'>
        <ul className='flex gap-10 text-lg uppercase font-semibold'>
          ...
          <li className={`${pathname.pathname === '/Grocery' ? 'font-bold' : 'font-normal'} hover:font-bold`}><Link to='cart' className='flex gap-2 justify-center items-center'><FaCartShopping/> Cart ({cart?.length})</Link></li>
          <li>{loggedInUser}</li>
        </ul>
      </div>
    </div>
